...
#define PROCNAME "/proc/cs60038_a2_18CS10066"
...
int main(){
    ...
    // int fd_lock = open("lock.txt", O_CREAT);
    ...
    for(int i=0;i<opers.size();i++)
    {
        ...
        {
            ret_val = ioctl(fd, PB2_EXTRACT_LEFT, &output[rct++]);
            if(ret_val < 0)
            {
        ...
        {
            ret_val = ioctl(fd, PB2_EXTRACT_RIGHT, &output[rct++]);
            if(ret_val < 0)
            {
                printf("PID %d : Failed to Read value!\n", curr_pid);
                close(fd);
